...
// Key for storing custom quotes in localStorage
...
export const getCustomQuotes = () => {
  const stored = localStorage.getItem(CUSTOM_QUOTES_KEY);
  return stored ? JSON.parse(stored) : [];
};

// Add a new custom quote
export const addCustomQuote = (quote) => {
...
export const removeCustomQuote = (quoteToRemove) => {
  const quotes = getCustomQuotes();
  const updatedQuotes = quotes.filter(quote => quote !== quoteToRemove);
  localStorage.setItem(CUSTOM_QUOTES_KEY, JSON.stringify(updatedQuotes));
